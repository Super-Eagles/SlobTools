...
不要保留寒暄、一次性细节、临时状态或纯过程性废话（例如只是查看/检查文件、简单问答、日常闲聊等）。
如果本轮对话不包含上述任何长期有价值的稳定信息，请直接返回空 JSON，即：{"summary":"","keywords":[]}。
只输出 JSON，格式必须是：
...
def run_chat_turn(
    ...
    summary_text = summary_pack["summary"].strip() if summary_pack else ""
    if not summary_text or summary_text in ("无", "none", "{}", "未提取到结构化摘要", "本轮对话完成，未提取到结构化摘要。"):
        mem_ids = []
    else:
        mem_ids = api.memorize(
            ...
            keywords=summary_pack["keywords"] if summary_pack else [],
            raw_q=user_text,
            raw_a=answer,
        )
